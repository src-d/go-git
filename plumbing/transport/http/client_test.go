...
func (s *FetchPackSuite) TestNewClientFactory(c *C) {
	e, err := transport.NewEndpoint("https://foo:bar@github.com/git-fixtures/basic")
	c.Assert(err, IsNil)

	roundTripper := &http.Transport{TLSClientConfig: &tls.Config{InsecureSkipVerify: true}}
	client := &http.Client{Transport: roundTripper}
	r := NewClientFactory(client)(e).(*Client)

	c.Assert(r.auth.String(), Equals, "http-basic-auth - foo:*******")
	c.Assert(r.c.Transport, Equals, roundTripper)
}
...
func (s *ClientSuite) TestSetAuth(c *C) {
	...
	r := NewClient(s.Endpoint)
	r.SetAuth(auth)
	c.Assert(auth, Equals, r.(*Client).auth)
}
